...
void generateLicense(struct UserInfo *user, FILE *file)
{
    ...
    FILE *registeredFile;
    registeredFile = fopen("registered_users.txt", "r");
    if (registeredFile != NULL)
    {
        char nic[20];
        while (fscanf(registeredFile, "%s", nic) != EOF)
        {
            if (strcmp(user->nic, nic) == 0)
            {
                fclose(registeredFile);
                system ("cls");
                printf("\nNIC already registered. Please enter a correct  NIC number. !!!!!\n\n");
                generateLicense(user, file); // Ask for NIC again
                return;
            }
        }
        fclose(registeredFile);
    ...
    // Save NIC to registered users file
    registeredFile = fopen("registered_users.txt", "a");
    fprintf(registeredFile, "%s\n", user->nic);
    fclose(registeredFile);
